...
public class PiServiceBean implements PiServiceBeanLocal {
	...
	public AccountPayDto doThreewayMatch(AccountPayDto dto1) {
		...
		ArrayList<ThreeWay_Matching_RequestItems> request = PIUtil.setThreewayRequest(dto1);
		...
		if (request.size() == 0) {
			...
			try {
				...
				ThreeWay_Matching_ResponseITEMS[] response = po_Details_Request_Sync_Out
						.threeWay_Matching_Request_Sync_Out((ThreeWay_Matching_RequestItems[]) request.toArray());
	...
	public ReponseDto postPoInvoices(AccountPayDto accpayDto) {
		...
		try {
			...
			// db.update(accpayDto);
			// logger.logDebug("[AP][PIServiceBean] [postPoInvoices] PostingLine
			// size :"+ request.getPostingLineItems().size());
			// logger.logDebug("[AP][PIServiceBean] [postPoInvoices]
			// MaterialLine size :" + request.getMATERIALPOSTING().size());
			...
			try {
				...
				if (!ServicesUtil.isEmpty(po_Posting_Response.getINVOICEDOCNUMBER()))
					accpayDto.getInvoiceDetailUIDto().setSapInvoiceNumber(po_Posting_Response.getINVOICEDOCNUMBER());
